    ...
    fun handle(rawInput: String): Result {
        ...
        if (containsAny(normalized, "목소리", "보이스", "voice")) {
            ...
            if (target != null) {
                ...
                return Result("Cinematic Voice를 $target 음성으로 변경했습니다. 이 문장부터 새 음성으로 재생합니다.", true)
            }
            if (containsAny(normalized, "테스트", "비교", "목록", "뭐 있어")) {
                return Result("Voice Lab에서는 marin, cedar, onyx, echo 네 가지 음성을 사용할 수 있습니다. 예를 들어 '목소리를 cedar로 바꿔 주세요'라고 말씀해 주세요.")
            }
        }
        ...
        if (normalized.startsWith("검색 ") || normalized.contains("검색해")) {
            ...
            return launch(intent, "$query 검색 화면을 열었습니다.", "검색 화면을 열 수 없습니다.")
        }
