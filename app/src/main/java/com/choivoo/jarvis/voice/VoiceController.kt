...
class VoiceController(
    ...
    private fun initRecognizer() {
        ...
        recognizer = SpeechRecognizer.createSpeechRecognizer(context).apply {
            setRecognitionListener(object : RecognitionListener {
                override fun onReadyForSpeech(params: Bundle?) { if (manualListening) onListeningStarted() }
                ...
                override fun onError(error: Int) {
                    ...
                    when (error) {
                        ...
                        else -> onError("음성 인식 오류가 발생했습니다. 코드 $error")
                    }
                }
    ...
    fun startListening() {
        ...
        try {
            ...
            manualListening = false
            onError("음성 인식기를 시작하지 못했습니다.")
        }
    }

    fun stopListening() { recognizer?.stopListening() }
    fun cancelListening() { manualListening = false; recognizer?.cancel() }
    ...
    fun speak(text: String) {
        ...
        when (provider) {
            ...
            else -> if (JarvisConfig.cloudEnabled) speakCloud(speech, fallbackToBasic = true, generation) else speakBasic(speech, "basic-auto-safe", generation = generation)
        }
    }
    ...
    private fun speakCloud(text: String, fallbackToBasic: Boolean, generation: Long) {
        ...
        if (file.exists() && file.length() > 256) { prefs.recordProvider("cloud-cache"); playCloud(file, text, fallbackToBasic, generation); return }
        thread(name = "jarvis-cloud-tts") {
            try {
                ...
                if (status !in 200..299) {
                    ...
                    connection.disconnect(); throw IllegalStateException("Cloud voice HTTP $status ${detail.take(100)}")
                }
    ...
    private fun playCloud(file: File, fallbackText: String, fallbackToBasic: Boolean, generation: Long) {
        ...
        try {
            ...
            player = MediaPlayer().apply {
                ...
                setOnCompletionListener { it.release(); if (player === it) player = null; if (isCurrent(generation)) onSpeakingFinished() }
                setOnErrorListener { p, _, _ ->
                    p.release(); if (player === p) player = null
                    if (isCurrent(generation)) { if (fallbackToBasic) speakBasic(fallbackText, "basic-cloud-playback-fallback", generation = generation) else onSpeakingFinished() }
                    true
                }
                prepareAsync()
            }
            ...
            if (!isCurrent(generation)) return
            if (fallbackToBasic) speakBasic(fallbackText, "basic-cloud-playback-fallback", generation = generation) else onSpeakingFinished()
        }
    }

    private fun getOrCreateNeural(): StandaloneNeuralTts { neural?.let { return it }; return StandaloneNeuralTts(context).also { neural = it } }
    ...
    private fun speakNeural(text: String, allowBasicFallback: Boolean, reason: String, generation: Long) {
        ...
        val engine = runCatching { getOrCreateNeural() }.getOrElse {
            ...
            else if (isCurrent(generation)) { onError("Neural Local Voice 초기화에 실패했습니다."); onSpeakingFinished() }
            return
        }
        if (!engine.isAvailable()) {
            if (allowBasicFallback) speakBasic(text, "basic-no-neural", generation = generation)
            else if (isCurrent(generation)) { onError("APK 내부 Neural Local 모델을 찾을 수 없습니다."); onSpeakingFinished() }
            return
        }
        prefs.recordProvider(reason)
        engine.speak(text = text, speed = prefs.getSpeed().toFloat(), onStart = {}, onDone = { main.post {
            if (!isCurrent(generation)) return@post
            prefs.recordProvider("neural"); onSpeakingFinished()
        } }, onError = { message -> main.post {
            if (!isCurrent(generation)) return@post
            prefs.recordProvider("neural-failed", message)
            runCatching { neural?.release() }; neural = null
    ...
    private fun speakBasic(text: String, label: String, attempt: Int = 0, generation: Long) {
        ...
        if (!basicReady) {
            if (!basicInitFinished && attempt < 16) { main.postDelayed({ if (isCurrent(generation)) speakBasic(text, label, attempt + 1, generation) }, 250L); return }
            onError("이 기기에서 British English 기본 TTS를 사용할 수 없습니다."); onSpeakingFinished(); return
        }
        val engine = tts ?: run { onSpeakingFinished(); return }
        engine.setSpeechRate(0.92f); engine.setPitch(0.86f)
        val utteranceId = "jarvis-en-gb-$generation"
        val result = engine.speak(text, TextToSpeech.QUEUE_FLUSH, null, utteranceId)
        if (result == TextToSpeech.ERROR && isCurrent(generation)) { onError("British English 기본 TTS 재생을 시작하지 못했습니다."); onSpeakingFinished() }
    }

    fun isNeuralReady(): Boolean = runCatching { context.assets.open("jarvis_tts/supertonic-3/tts.json").close(); true }.getOrDefault(false)
    ...
    private fun stopPlaybackOnly() {
        ...
        runCatching { neural?.stop() }; runCatching { tts?.stop() }
    }

    fun stopSpeaking() { speechGeneration.incrementAndGet(); stopPlaybackOnly() }

    fun destroy() {
        manualListening = false
        recognizer?.destroy(); recognizer = null
        stopSpeaking(); runCatching { neural?.release() }; neural = null
