...
class JarvisAssistantEngine(private val context: Context) {
    ...
    suspend fun process(command: String, history: List<BrainClient.Turn> = emptyList()): Result {
        ...
        handleTaskCommand(command, normalized)?.let { return it }
        handleCalendarCreate(command, normalized)?.let { return it }
        ...
        if (containsAny(normalized, "날씨", "기온", "비 와", "비오", "우산")) {
            ...
            val snapshot = weather.current().getOrElse {
                return Result("현재 위치 또는 날씨 정보를 가져오지 못했습니다. 위치 서비스를 확인해 주세요.")
            }
        ...
        if (containsAny(normalized, "모닝 브리핑", "아침 브리핑", "오늘 브리핑", "오늘 요약")) {
            ...
            return Result("좋은 아침입니다. 배터리는 ${battery}%입니다. $weatherText $calendarText $taskText $notificationText")
        }
    ...
    private fun handleTaskCommand(original: String, normalized: String): Result? {
        ...
        if (containsAny(normalized, "할 일 추가", "할일 추가", "할 일에", "할일에")) {
            ...
            tasks.add(title)
            return Result("할 일에 '$title' 항목을 추가했습니다.", true)
        }
        ...
        if (containsAny(normalized, "할 일 완료", "할일 완료", "완료 처리")) {
            ...
            return Result("'${completed.title}' 항목을 완료 처리했습니다.", true)
        }
        return null
    }
    ...
    private fun handleCalendarCreate(original: String, normalized: String): Result? {
        ...
        val hourValue = hour?.groupValues?.getOrNull(2)?.toIntOrNull() ?: return Result("일정을 추가하려면 시간을 함께 말씀해 주세요. 예: '내일 오후 3시에 학원 일정 추가해 주세요'.")
        ...
        return try {
            ...
            Result("캘린더에 '$title' 일정을 추가할 준비를 했습니다. 화면에서 저장을 확인해 주세요.", true)
        } catch (_: Exception) {
